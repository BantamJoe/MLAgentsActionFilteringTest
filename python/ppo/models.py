...
class PPOModel(object):
    ...
    def create_continuous_state_encoder(self, s_size, h_size, num_streams, activation, num_layers):
        ...
        if action_filter:
            self.state_original = self.state_in
            self.action_filter = tf.cast(self.state_original[:,:self.a_size] > 0.2, dtype=tf.float32)
            self.state_in_remain = self.state_original[:,self.a_size:]
        else:
            self.state_in_remain = self.state_in
        ...
        if self.normalize:
            ...
            self.normalized_state = tf.clip_by_value((self.state_in_remain - self.running_mean) / tf.sqrt(
            ...
            self.normalized_state = self.state_in_remain
        streams = []
        for i in range(num_streams):
            hidden = self.normalized_state
            for j in range(num_layers):
                hidden = tf.layers.dense(hidden, h_size, use_bias=False, activation=activation, name= "hidden_{}{}".format(i,j))
            streams.append(hidden)
        return streams
...
class DiscreteControlModel(PPOModel):
    def __init__(self, lr, brain, h_size, epsilon, beta, max_step, normalize, num_layers):
        ...
        self.batch_size = tf.placeholder(shape=None, dtype=tf.int32, name='batch_size')
        self.policy_original = tf.layers.dense(hidden, a_size, activation=None, use_bias=False,
                                      kernel_initializer=c_layers.variance_scaling_initializer(factor=0.01), name='policy_original')
        
        if action_filter:
            self.policy = tf.multiply(self.policy_original, self.action_filter)
            self.policy = tf.add(self.policy, tf.multiply(tf.subtract(self.action_filter,1), invalid_action_factor), name="policy")
        else:
            self.policy = self.policy_original
